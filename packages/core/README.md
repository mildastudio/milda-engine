...
every Milda tool operates on between the [`milda`](../../milda) language and the
generators.

Where the `milda` package defines the canonical language and prelude, `@mildastudio/core`
...
`0.x` — unstable until `1.0`. Depends on [`milda`](../../milda).
